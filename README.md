...
5. 在设置顶部可开关“开机自启动”；也可分别调整文本、图片和文件的保留条数与保存策略。“磁盘缓存”下方可一键清理 `~/Library/Application Support/Barclip/` 中的缓存文件。

“清空历史”只删除当前分类在 Barclip 中的记录，不会修改系统剪贴板；“清空暂存”只删除文件引用。“一键清理全部磁盘缓存”会在确认后删除该缓存目录中的内容，并清空文本、图片和文件暂存记录，但不会删除原文件或重置其他设置。
...
持久化数据未加密；删除应用不会自动删除上述目录。如需移除 Barclip 记录，可在设置中点击“一键清理全部磁盘缓存”，或在退出 Barclip 后手动删除该目录。

...
关于页「项目仓库」同时列出 GitHub 和 Gitee。设置页可开关「登录时打开」。状态以系统登录项为准，使用 `SMAppService.mainApp` 注册当前应用，不写 UserDefaults，也不嵌入登录助手。版权持有人显示为 **Yuntao Chen**（`© 2026 Yuntao Chen`）。关于页开发者姓名仍为陈云涛。文件中转站已接入。图片历史支持点击选中并按空格预览。进出设置使用固定面板尺寸。真实登录注销、系统登录项弹窗和图片历史键盘交互仍待手动验收。

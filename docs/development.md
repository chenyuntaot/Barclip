...
设置页可开关「登录时打开」。状态以系统登录项为准，使用 `SMAppService.mainApp` 注册当前应用，不写 UserDefaults，也不嵌入登录助手。版权持有人显示为 **Yuntao Chen**（`© 2026 Yuntao Chen`）。关于页开发者姓名仍为陈云涛。文件中转站已接入。进出设置使用固定面板尺寸。Debug XCTest 含开机启动模拟服务断言；真实登录注销和系统登录项弹窗仍待手动验收。

## 2026-09-19：开机自启动

- 设置页增加「登录时打开」开关。打开后由系统在用户登录时启动 Barclip；关闭则注销登录项。
- 系统状态为需要批准时，开关保持打开，并提供「打开登录项设置」。安装位置无法注册（常见于 Xcode 直接运行）时提示把应用放到应用程序文件夹。
...
- 测试：Debug XCTest 58 项通过、0 失败；新增 6 项开机启动模拟服务测试，覆盖打开、关闭、待批准、安装位置无法注册、系统失败与只读刷新。测试不改本机真实登录项。从应用程序文件夹打开后的系统登录项、注销再登录，以及 Xcode 直接运行时的 notFound 提示仍待手动验收。
...
| 登录时打开 | 设置页开关。打开后由 macOS 登录项在用户登录时启动 Barclip。需要批准时提示前往系统设置；当前拷贝无法注册时提示移到应用程序文件夹。不保存在应用偏好设置里 |
...
17. 将 Barclip 放到应用程序文件夹后打开设置，打开「登录时打开」；在系统设置 → 通用 → 登录项中确认出现 Barclip。注销再登录后菜单栏应自动出现图标。关闭开关后登录项应消失。若系统要求批准，点「打开登录项设置」完成允许。从 Xcode 直接运行时若无法注册，应看到安装位置提示。

...
关于页「项目仓库」同时列出 GitHub 和 Gitee。设置页可开关「登录时打开」。状态以系统登录项为准，使用 `SMAppService.mainApp` 注册当前应用，不写 UserDefaults，也不嵌入登录助手。版权持有人显示为 **Yuntao Chen**（`© 2026 Yuntao Chen`）。关于页开发者姓名仍为陈云涛。文件中转站已接入。图片历史支持点击选中并按空格预览。侧栏未选中项使用深黑色文字和图标，选中项使用深蓝色并保留液态玻璃背景。进出设置使用固定面板尺寸。真实登录注销、系统登录项弹窗和图片历史键盘交互仍待手动验收。
...
需求范围：macOS 原生、菜单栏常驻、Dock 不展示、文本与图片历史、左侧分类导航、一键清空当前分类、容量设置、设置页中的两种保存策略、设置页中的登录时打开、设置页底部的程序版本 / 关于我们 / 版权，以及简体中文 / 英文界面（跟随系统语言）。
...
| 设置 | 进入后侧栏收起，左右合为一块，左上角返回；面板外框尺寸与历史/文件页相同，避免菜单栏窗口缩放。设置选择跨重启保留，并展示带复制按钮的实际磁盘缓存目录。可开关登录时打开。底部显示程序版本、关于我们和版权，点击进入关于页 |
| 关于 | 显示程序版本、开发者陈云涛、联系邮箱、项目仓库（GitHub 与 Gitee）和版权 © 2026 Yuntao Chen。邮箱和仓库用系统默认应用打开。返回回到设置 |
| 登录时打开 | 设置页开关。打开后由 macOS 登录项在用户登录时启动 Barclip。需要批准时提示前往系统设置。不保存在应用偏好设置里 |
...
- Xcode Debug 测试构建与 Release arm64 + x86_64 Build：通过，Swift 6 编译和类型检查通过。当前为 59 项 XCTest，含文件暂存与图片历史预览。
- XCTest：59 项通过，无失败；涵盖文本排序、去重、空白保真、容量、清空后不回填、重复操作、非文本与超大文本、图片排序去重、图文独立容量、图片复制失败、图片选中态与内存预览、访问拒绝、读取重试、复制失败、监控停止/重启、截图样式 TIFF/PNG+fileURL 收录与纯文件跳过、关于页元数据、设置页底部入口高度与应用图标配置。
...
17. 将 Barclip 放到应用程序文件夹后打开设置，打开「登录时打开」；在系统设置 → 通用 → 登录项中确认出现 Barclip。注销再登录后菜单栏应自动出现图标。关闭开关后登录项应消失。若系统要求批准，点「打开登录项设置」完成允许。

...
需求范围：macOS 原生、菜单栏常驻、Dock 不展示、文本与图片历史、左侧分类导航、一键清空当前分类、容量设置、设置页中的两种保存策略，以及设置页底部的程序版本 / 关于我们 / 版权。
...
- 版本号来自 `MARKETING_VERSION` / `CURRENT_PROJECT_VERSION`（当前 1.0 / 1），版权写入生成的 Info.plist。

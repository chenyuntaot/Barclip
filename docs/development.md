...
关于页「项目仓库」同时列出 GitHub 和 Gitee。设置页顶部显示标题，第一项为「开机自启动」，可分别调整文本、图片和文件容量，并可确认后一键清理 `~/Library/Application Support/Barclip/` 中的全部磁盘缓存；三个 Slider 不再绘制密集步进刻度。开机自启动状态以系统登录项为准，使用 `SMAppService.mainApp` 注册当前应用，不写 UserDefaults，也不嵌入登录助手。版权持有人显示为 **Yuntao Chen**（`© 2026 Yuntao Chen`）。关于页开发者姓名仍为陈云涛。文件中转站已接入。图片历史支持点击选中并按空格预览。侧栏未选中项使用深黑色文字和图标，选中项使用深蓝色并保留液态玻璃背景。进出设置使用固定面板尺寸。真实登录注销、系统登录项弹窗、图片历史键盘交互和全部缓存清理确认框仍待手动验收。

## 2026-09-20：图片历史选中与空格预览

- 点击图片记录仍会写回系统剪贴板，同时记录为当前选中项；复制导致该图片重新置顶并生成新记录标识时，选中状态会跟随新的标识，不会丢失。
- 选中图片复用文件页同一套液态玻璃效果，不显示额外的蓝色背景或描边，并暴露辅助功能选中状态。图片页获得键盘焦点后，按空格在应用内浮动窗口预览；重复请求预览同一项时会关闭已有预览。
- 预览直接使用历史记录中的 PNG 数据，不创建临时文件；复用文件暂存已有的浮动预览窗口。无选中项或图片数据无法解码时不打开空窗口。
- 涉及：`ClipboardMenuView.swift`、`FileQuickLookController.swift`、`Localizable.xcstrings`、`FileStagingTests.swift`、`ViewRenderingTests.swift`、README、开发与架构文档。
- 验证：完整 Debug XCTest 59 项通过、0 失败；Release arm64 + x86_64 Build 通过。液态玻璃样式调整后再次定向执行 7 项 `ViewRenderingTests`，全部通过。新增覆盖有效/无效内存图片预览、同图再次预览关闭、英文文案，以及图片液态玻璃选中态浅色渲染；已目视检查选中态快照无蓝色描边。真实 MenuBarExtra 中点击图片后按空格仍待用新构建手动验收。

## 2026-09-20：文件暂存条改为圆角矩形液态玻璃

- 菜单栏附近的「拖到此处暂存」弹窗改为简单的圆角矩形，不再使用箭头和自定义气泡路径。
- macOS 26 使用原生 `glassEffect(.regular)`，macOS 14 / 15 回退为 `ultraThinMaterial`；默认状态依靠玻璃自身边缘，不再额外画灰色描边，文件进入投放区时仍显示强调色描边和文字反馈。
- 透明、非激活、状态栏层级和展开动画保持不变，没有引入新的窗口或依赖。
- 涉及：`DropShelfView.swift`、渲染测试、开发与架构文档。

## 2026-09-20：关于页增加 Gitee 仓库

- 「项目仓库」在 GitHub 下方增加 Gitee：`https://gitee.com/chenyuntao-0123/barclip`。两个地址都用系统默认应用打开。GitHub / Gitee 名称和地址不翻译。
- 涉及：`AppInfo.swift`、`ClipboardAboutView.swift`、`ViewRenderingTests.swift`、README、开发文档。
- 测试：`ViewRenderingTests` 通过，`testAppInfoExposesAboutMetadata` 断言 GitHub 与 Gitee 地址同时存在。关于页真实点击仍待重新启动应用后目视。

## 2026-09-19：开机自启动

- 设置页增加「登录时打开」开关。打开后由系统在用户登录时启动 Barclip；关闭则注销登录项。
- 系统状态为需要批准时，开关保持打开，并提供「打开登录项设置」。安装位置无法注册时不再显示红色说明。
- 不把开关缓存在 UserDefaults，避免和系统设置里的登录项不一致。测试使用模拟服务，不会改本机真实登录项。
- 涉及：`LaunchAtLoginService.swift`、`LaunchAtLoginStore.swift`、`ClipboardSettingsView.swift`、`ClipboardHistoryApp.swift`、本地化、测试与文档。
- 测试：开机启动模拟服务覆盖打开、关闭、待批准、安装位置无法注册且不展示位置标语、系统失败与只读刷新。测试不改本机真实登录项。从应用程序文件夹打开后的系统登录项和注销再登录仍待手动验收。
...
- 拖文件时：监听鼠标拖动，且仅当 `NSPasteboard.Name.drag` 的 changeCount 相对上次会话增加并带有文件 URL 时才弹出非激活 `NSPanel`。普通点击、按住鼠标或剪贴板上残留的旧文件引用不会打开暂存条。不打开 `MenuBarExtra`，也不激活应用。圆角矩形从 Barclip 菜单栏图标下方展开；找不到图标时落在当前屏幕右上靠近菜单栏处，不居中。
...
需求范围：macOS 原生、菜单栏常驻、Dock 不展示、文本与图片历史、左侧分类导航、一键清空当前分类、容量设置、设置页中的两种保存策略、设置页顶部的开机自启动、设置页底部的程序版本 / 关于我们 / 版权，以及简体中文 / 英文界面（跟随系统语言）。
...
| 文件暂存 | 拖入只记 bookmark，原件不动。可从面板或拖文件时出现的菜单栏圆角矩形暂存条放入。文件页两列图标网格。拖出复制、空格预览、双击打开。原件删除后标记丢失。Finder 复制的纯文件仍不进入文本/图片历史 |
| 排序与去重 | 新记录置顶；相同文本或相同 PNG 再次复制时移到顶部；同一文件再拖入暂存时置顶 |
| 文本保真 | 保留原始空格、换行和 Unicode；空白文本忽略 |
| 点击记录 | 文本与图片都会写回系统剪贴板并显示已复制提示。图片同时成为当前选中项，按空格可在浮动窗口预览 |
| 清空 | 文本/图片只清空当前分类的应用内历史；文件页清空暂存引用；设置页可确认后一键清空 `Application Support/Barclip` 目录内容（文本、图片 sidecar、文件暂存 JSON 及残留文件），并同时清空应用内对应记录。均不删除原文件或修改系统剪贴板 |
| 容量 | 1...200 的任意整数，默认 50；文本、图片和文件各自保留该数量，调小立即移除最旧记录 |
| 退出后清空 | 默认模式，仅在当前进程保留；切换到此模式立即删除磁盘缓存 |
| 重启后保留 | 将当前及后续历史写进用户 Application Support/Barclip，启动时恢复；删除应用后仍保留 |
| 设置 | 进入后侧栏收起，左右合为一块，左上角返回，页面顶部显示“设置”；面板外框尺寸与历史/文件页相同，避免菜单栏窗口缩放。设置第一项为开机自启动，其余选择跨重启保留，并展示带复制按钮的实际磁盘缓存目录及全部清理按钮。底部显示程序版本、关于我们和版权，点击进入关于页 |
| 关于 | 显示程序版本、开发者陈云涛、联系邮箱、项目仓库（GitHub 与 Gitee）和版权 © 2026 Yuntao Chen。邮箱和仓库用系统默认应用打开。返回回到设置 |
| 开机自启动 | 设置页最上方开关。打开后由 macOS 登录项在用户登录时启动 Barclip。需要批准时提示前往系统设置。不保存在应用偏好设置里 |
...
- Xcode Debug 测试构建与 Release arm64 + x86_64 Build：通过，Swift 6 编译和类型检查通过。当前为 63 项 XCTest，含文件暂存、图片历史预览、容量滑块视觉回归和全部缓存目录清理。
- XCTest：63 项通过，无失败；涵盖文本排序、去重、空白保真、独立容量、清空后不回填、全部缓存目录清理且保留原文件、重复操作、非文本与超大文本、图片排序去重、图片复制失败、图片选中态与内存预览、访问拒绝、读取重试、复制失败、监控停止/重启、截图样式 TIFF/PNG+fileURL 收录与纯文件跳过、关于页元数据、设置页标题、滑块刻度黑线、底部入口高度与应用图标配置。
...
3. 用系统截图（⌘⇧3 / ⌘⇧4，或 ⌘⌃⇧3 / ⌘⌃⇧4 直接进剪贴板）后打开「图片」，确认缩略图出现；点击后确认液态玻璃选中效果出现且没有蓝色描边，并可在文稿中粘贴，再按空格验证浮动预览。
...
14. 将系统语言改为 English，确认面板、设置、关于页和退出确认框为英文；Barclip、开发者陈云涛、版权 Yuntao Chen、邮箱、GitHub 和 Gitee 地址保持原样。改回简体中文后界面恢复中文。
...
17. 将 Barclip 放到应用程序文件夹后打开设置，打开顶部的「开机自启动」；在系统设置 → 通用 → 登录项中确认出现 Barclip。注销再登录后菜单栏应自动出现图标。关闭开关后登录项应消失。若系统要求批准，点「打开登录项设置」完成允许。
...
- 单条文本上限 1 MiB，单条图片上限 30 MiB；不包含搜索、收藏、全局快捷键、云同步和自动粘贴。
...
- 关于页展示 Barclip 版本、开发者陈云涛、联系邮箱 `chenyuntao0123@icloud.com`、项目仓库 GitHub `https://github.com/chenyuntaot/Barclip` 与 Gitee `https://gitee.com/chenyuntao-0123/barclip` 和版权 `© 2026 Yuntao Chen`。邮箱与仓库用系统默认应用打开。
...
## 2026-09-21：文本、图片、文件独立保留条数

- 实现状态：已完成。设置页将原先共用的固定选项 Picker 改为文本、图片、文件三行独立设置；每行使用 macOS 原生 SwiftUI `Slider` 和右侧圆角数字 `TextField`，允许选择或输入 1...200 的任意整数，例如 25、10、26。
- 实现方式：`ClipboardStore` 分别维护文本和图片容量，`FileStagingStore` 独立维护文件容量；三个值分别写入 UserDefaults。列表记录、新记录写入和持久化恢复均按所属类型裁剪，调小后立即生效。文本或图片页顶部的计数上限跟随当前类型显示。
- 兼容决策：升级后若尚无独立设置，读取旧版 `historyCapacity` 作为三个类型的初始值，避免用户原设置丢失；用户首次调整后写入新的独立键。继续沿用原 200 条上限，不新增依赖。
- 涉及文件：`ClipboardSettingsView.swift`、`ClipboardMenuView.swift`、`ClipboardStore.swift`、`FileStagingStore.swift`、相关 XCTest、`Localizable.xcstrings`、README 与架构文档。
- 测试：Debug XCTest 60 项通过、0 失败；Release arm64 + x86_64 Build 通过。新增或更新测试覆盖文本/图片独立容量、任意整数 26、越界值拒绝、旧设置迁移、文件容量持久化、即时裁剪和英文说明。
- UI 检查：已渲染浅色/深色及中英文快照，并目视检查 360pt 宽设置页；中文三行对齐，英文 Images 不换行，数字输入框可见，底部关于入口仍完整显示。
- 已知问题与后续：自动化未模拟真实鼠标拖动 Slider 或键盘编辑 TextField；真实 MenuBarExtra 中的输入焦点、回车提交和拖动手感仍需手动验收。Release 构建仍有 `FileStagingTransfer.swift` 的既有 unused result / conditional downcast 警告，与本功能无关。

## 2026-09-21：设置页标题、滑块黑线与全部缓存清理

- 实现状态：已完成。设置页顶部增加“设置”标题；磁盘缓存区域增加“一键清理全部磁盘缓存”原生按钮，执行前显示破坏性确认。
- 黑线根因与修复：容量 Slider 的 `step: 1` 会让 macOS 为 1...200 的每个步进绘制刻度，密集刻度在轨道下方合并成黑线。移除 `step`，继续在 Binding setter 中四舍五入为整数，因此条数仍只会写入整数。
- 清理范围：确认后同时清空文本历史、图片历史、文件暂存引用及对应落盘文件；等待两个 Store 的保存任务完成后显示成功或失败。不会删除用户原文件，不修改保留条数、保存策略或其他偏好。
- 技术选择：当时复用两个 Store 的 `clear()` 编排落盘删除，确认框使用 SwiftUI `.alert`。该方案在 MenuBarExtra 中经常无法真正执行，且只删除已知快照，目录中的残留文件会留下。
- 涉及文件：`ClipboardSettingsView.swift`、`Localizable.xcstrings`、`ViewRenderingTests.swift`、README、开发与架构文档。
- 测试：新增视觉回归，先验证带 `step: 1` 的三个滑块均会因轨道下黑线而失败，移除后通过；新增全部缓存清理测试。真实菜单栏中的确认流程当时未验收。

## 2026-09-21：一键清理改为直接清空缓存目录

- 实现状态：已完成代码与自动化测试。用户确认后会清空 `~/Library/Application Support/Barclip/` 目录内容，并同步清空应用内文本、图片和文件暂存记录。
- 根因：设置页使用 SwiftUI `.alert`，在菜单栏窗口中经常无法弹出或确认后不执行；清理只走 Store 的空快照保存，不会删除目录里的残留文件。
- 修复：确认改用与退出流程相同的 `NSAlert.runModal()`；`HistoryRepository.wipeCacheDirectory()` 先写入 `.migration-complete`，再删除该目录中其余全部项目（`history.json`、`file-staging.json`、`images/`、`.DS_Store` 等）。不删除用户原文件，不重置容量、保存策略或其他偏好。
- 涉及文件：`HistoryRepository.swift`、`ClipboardStore.swift`、`FileStagingStore.swift`、`ClipboardSettingsView.swift`、`ViewRenderingTests.swift`、`HistoryPersistenceTests.swift`、README、开发与架构文档。
- 测试：更新全部缓存清理用例，覆盖文本、图片 sidecar、文件暂存和残留文件；新增仓库层目录擦除测试，确认只保留迁移标记且不会重新导入。自动化不操作真实用户缓存。真实菜单栏中点确认后查看该目录仍待手动验收。

## 2026-09-21：开机自启动改名并置顶

- 实现状态：已完成。设置页开关文案由「登录时打开」改为「开机自启动」，并移到设置页最上方，位于保留条数之前。英文为 Launch at Startup。实现仍使用系统登录项，不改注册方式。
- 涉及文件：`ClipboardSettingsView.swift`、`Localizable.xcstrings`、`ViewRenderingTests.swift`、README、开发与架构文档。
- 测试：更新英文目录断言，并扩大容量滑块视觉检测的纵向范围，以适应开机自启动置顶后的新布局。真实菜单栏置顶位置仍待重新打开新构建后目视。

## 2026-09-21：侧栏按钮在左右边界间居中

- 实现状态：已完成。侧栏按钮、图标和中间横线在窗口左缘到竖向分隔线之间水平居中。
- 根因：面板四周都有 16pt 内边距，侧栏轨道只有 64pt，按钮在轨道内居中后，相对整列（内边距 + 轨道）会偏右约 8pt。
- 修复：侧栏视觉宽度覆盖左侧内边距，布局宽度仍为 64pt，进出设置的外框尺寸不变，避免液态玻璃残影。
- 涉及文件：`ClipboardMenuView.swift`、开发与架构文档。
- 测试：`ViewRenderingTests` 中的固定窗口尺寸和菜单渲染。真实菜单栏目视仍待用新构建确认。

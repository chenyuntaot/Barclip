...
| 设置 | 进入后侧栏收起，左右合为一块，左上角返回；设置选择跨重启保留，并展示可选中复制的实际磁盘缓存目录。底部显示程序版本、关于我们和版权，点击进入关于页 |
...
12. 打开设置，确认显示当前用户的完整磁盘缓存目录且可选中复制；在 Finder 的“前往文件夹”中粘贴后能进入该目录。确认底部显示程序版本、关于我们和版权；点击进入关于页后返回设置，再返回历史。点击邮箱和仓库能打开对应应用。
